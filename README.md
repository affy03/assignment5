...
> [!NOTE]
URLとURIの違い        
URL：ウェブサイトや画像ファイルの場所を特定する具体的なアドレス    
URI：書籍や学術論文を一意に特定するための識別子  
...
> [!NOTE]
クエリとは：SQL言語を使ったデータベースへの命令文、もしくは、検索キーワード。キーと値で記される。
SQLとは：データベースとやり取りするときに使う言語。
...
> [!NOTE]
パスとは：URLの一部。リソースがどの場所に存在するかを指定する。Windowsは（C:\Windows\system32\のように）\で、MacやLinuxは（/user/bin/pearlのように）/で区切る。
...
> [!NOTE]
> レスポンス(HTTPレスポンス)とは：WebサーバーからWebブラウザに対して出される返事。
